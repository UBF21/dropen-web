...
export function productImgSrc(url: string | null | undefined, width = 600, quality = 80): string {
  const DEFAULT =
  ...
  if (url.includes(SUPABASE_STORAGE)) return `${url}?width=${width}&quality=${quality}`
  return url
}
